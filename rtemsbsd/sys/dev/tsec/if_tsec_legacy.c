...
if_t         		*ifp;
	...
	ifp = &theTsecEths[unit-1].ifp;
	
	if ( theTsecEths[unit-1].was_init ) {
...
BSP_tsec_detach(struct tsec_private *mp)
{
	...
	mp->sc->was_init = false;

	return 0;
}

static int
rtems_tsec_detach(struct tsec_softc* sc)
{
	return BSP_tsec_detach(&sc->pvt);
}
...
static void tsec_daemon(uintptr_t arg)
{
struct tsec_softc	*sc;
struct ifnet		*ifp;
rtems_event_set		evs;
	for (;;) {
		rtems_event_receive( EV_MSK, RTEMS_WAIT | RTEMS_EVENT_ANY, RTEMS_NO_TIMEOUT, &evs );
		evs &= EV_MSK;
...
static driver_t legacy_tsec_driver = {
	"legacy_tsec",
	legacy_tsec_methods,
	sizeof(struct tsec_softc)
};

DRIVER_MODULE(legacy_tsec, nexus, legacy_tsec_driver, 0, 0);
DRIVER_MODULE(miibus, legacy_tsec, miibus_driver, 0, 0);

MODULE_DEPEND(legacy_tsec, nexus, 1, 1, 1);
MODULE_DEPEND(legacy_tsec, ether, 1, 1, 1);
...
#endif // LIBBSP_POWERPC_MVME3100_BSP_H